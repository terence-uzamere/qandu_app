...
class QuestionDetailView(DetailView):
  ...
  def get_context_data(self, **kwargs):
    ...
    question = question.object.get(id=self.kwargs['pk'])
    answers = Answer.object.filter(question=question)
    context['answers'] = answers
    user_answers = Answer.object.filter(question=question, user=self.request.user)
    context['user_answers'] = user_answers
    user_vote = Answer.objects.filter(vote__user=self.request.user)
    context['user_votes'] = user_votes
    return context
...
class VoteFormView(FormView):
  ...
  def form_valid(self, form):
    ...
    try:
      answer = answer.object.get(pk=form.data["answer"])
...
class UserDetailView(DetailView):
  ...
  def get_context_data(self, **kwargs):
    ...
    questions = Question.object.filter(user=user_in_view).exclude(visibility=1)
...
class UserUpdateView(UpdateView):
  ...
  def get_object(self, *args, **kwargs):
    object = super(UserUpdateView, self).get_objects(*args, **kwargs)
